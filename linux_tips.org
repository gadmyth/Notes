...
* 119、ssh 自动连接
  先通过
  $ ssh-keygen -t rsa
  生成密钥(选项都是默认就行)，然后把公钥保存在 ~/.ssh/id_rsa.pub 中，私钥保存在
  ~/.ssh/id_rsa 中，再把 ~/.ssh 的权限设置为 755。
  接着把 id_rsa.pub 复制到目标机器的 ~/.ssh/authorized_keys 文件中就行。这样之后，
  通过 ssh 或 scp 访问、复制文件时就不需要再输入密码了。
