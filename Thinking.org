...
** 20、查看网站对爬虫的限制 robots.txt
   可通过如下例的格式来查看:

   http://www.dangdang.com/robots.txt
   ...
   解释: The "User-agent: *" means this section applies to all robots. The
   "Disallow: /" tells the robot that it should not visit any pages on the site.
   "Allow: /reader/play" tells the robot that it could visit the pages of
   '/reader/play'.

   There are two important considerations when using '/robots.txt':
   1) robots can ignore your /robots.txt. Especially malware robots that scan
      the web for security vulnerabilities, and email address harvesters used by
      spammers will pay no attentino.
   2) the /robots.txt file is a publicly avaiable file. Anyone can see what
