...
** 22、xrange([start,] stop [, step])
    作用同 range()，但返回的是 xrange object，比 range() 速度快同时更节省内存。
    它一般只用于 for 遍历中。
** 23、map(function, sequence [, sequence, ...]) -> list
** 24、filter(function or None, sequence) -> list, tuple, or string
** 25、chr(i) -> Character
    0 <= i <= 256
    返回数字 i 对应的 ASCII 字符
** 26、ord(c) -> integer
...
** 8、排序输出 dict 的 key 值
    ...
    的 key 值有序，且用这个函数后，返回的是排序后的 key 值 tuple，但我发现新版本
    的 Python 在建立后会自动对 key 值排序，故我觉得无须再使用这个函数获得有序的
    key 序列。
** 9、键冲突
...
** 1、基本用法
   ...
   The one weakness of list comps is that all of teh data have to be made
...
** 1、Basic concept and format
   A generator si a specialized function that allows you to return a value and
   "pause" the execution of that code and resume it at a later time.
   Instead of building a list with valuse, generator expressions return a
...
** Seach Path
   ...
   stored in the *sys* module as the *sys.path* variable. Rather tahn a single
...
** Method and self
   The *self* keyword must be present in all method declarations. That argument,
   representing the instance object, is passed to the method implicitly by the
   interpreter when you invoke a method on an instance.
   It is all part of Python's philosophy of being explicitly clear.
   Requiring the instantce only applies to regular methods and not static or
   class methods, although the latter requires the class the class rather than
   the instance.
** 在子类中调用基类的构造函数
...
*** Abstraction/Implementation
    /Abstracttion/ refers to the modeling of essential aspects, behavior, and
...
*** Instance Attributes versus Class Attributes
    ...
    nay particular instances like instance attributes are. Such values are also
...
*** Overriding Methods through Inheritence
    ...
    class C(P):
        def __init__(self):
	    super(C, slef).__init__(self)
	    pritn "Calling C's constructor"
...
** 2、Detecting and Handling Exceptions
   ...
   Exceptions.
   There are two main forms fo the /try/ statement: *try-except* and
   ...
   *try-except* statements allow one to dectect and handle exceptions. There is
   ...
   when no exceptions are detected. Meanwhile, *tyr-finally* statements allow
...
** 
* 47、在 nginx 中配置和使用 python 程序
** 通过 uwsgi
*** 配置 nginx
   安装号 uwsgi 后，在 nginx.conf 中添加如下语句:
